...
| Field | Value |
| --- | --- |
| Date/time | 2026-08-13, 06:29–09:18 (Asia/Ho_Chi_Minh) |
...
| Result | Pass cho checklist Android tối thiểu; các dòng `Blocked` bên dưới chưa được tuyên bố đã kiểm thử |
...
| Habit | Boolean complete/undo | One tap updates progress and dashboard | Blocked | Complete đã đạt (checked, streak 1); thao tác undo chưa chạy trong lượt này nên không nâng thành Pass. |
| Habit | Count +/- | Value changes once; cannot fall below zero | Blocked | Increment thực tế `0 → 2/3` đạt; decrement và biên zero chưa chạy trên thiết bị. |
...
| Project | Project/task CRUD | Create/edit/complete/archive persist after refresh | Blocked | Tạo project `QA Launch`, tạo/sửa task thành `Device QA task updated` và link Focus đạt; complete/archive toàn bộ project flow chưa chạy trên thiết bị. |
...
| Break | Short/long break | Correct break is selected and restored after app restart | Blocked | Short break 5 phút và long break 15 phút sau phiên thứ tư đều được chọn đúng; chưa force-stop/reopen trong chính pha break. |
...
| Notification | Break completion | Fires at break end; tap opens Focus | Blocked | Break schedule/cancel path có unit test; chưa chờ một break notification thật phát trên thiết bị. |
| Notification | Habit DAILY/WEEKDAYS | Fires on configured time/day; tap opens Habits | Blocked | Native schedule code có test; chưa chờ reminder habit thật phát trên thiết bị. |
...
| Accessibility | Font scale + screen reader | Timer/progress/actions have useful labels; layout remains usable | Blocked | Font scale 1.3 đã kiểm Today/Focus/Settings; tab bar được sửa dùng bottom inset và còn đủ icon/label. Timer/progress/button labels có trong accessibility hierarchy. TalkBack gesture/audible output chưa chạy. Font scale đã khôi phục `1.0`. |
