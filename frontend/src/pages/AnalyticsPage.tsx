...
import { ErrorState, LoadingBlock, PageHeader, ProgressBar, Select, Stat } from '../components/ui'
...
export function AnalyticsPage() {
  const { preset, setPreset, range } = useDateRange('30d')
  ...
  return (
    <div>
      <PageHeader eyebrow="Nhìn lại có chủ đích" title="Phân tích nhịp làm việc" description="Dữ liệu thật từ các phiên tập trung và lần hoàn thành thói quen, trong khoảng bạn chọn." action={<Select label="Khoảng thời gian" value={preset} onChange={(event) => setPreset(event.target.value as typeof preset)}><option value="7d">7 ngày</option><option value="30d">30 ngày</option><option value="today">Hôm nay</option></Select>} />
